...
function parsePgVerboseObjects(output = "") {
  ...
  for (const line of lines) {
    const tableDumpMatch = line.match(/table\s+"?([^"\s]+(?:\.[^"\s]+)?)"?/i);
    const sequenceMatch = line.match(/sequence\s+"?([^"\s]+(?:\.[^"\s]+)?)"?/i);

    if (tableDumpMatch) {
      tables.push(tableDumpMatch[1]);
    }

    if (sequenceMatch) {
      sequences.push(sequenceMatch[1]);
    }
  }

  return {
    tables: uniqueItems(tables),
    sequences: uniqueItems(sequences),
  };
}
...
async function parseDumpFileObjects(filepath, env) {
  try {
    const { stdout } = await execAsync(`pg_restore -l "${filepath}"`, {
      env,
    ...
    for (const line of lines) {
      ...
      const sequenceMatch = line.match(/\bSEQUENCE\b\s+("?[\w]+"?)\s+("?[\w]+"?)/i);

      if (tableMatch) {
        tables.push(`${tableMatch[1].replace(/"/g, "")}.${tableMatch[2].replace(/"/g, "")}`);
      }
      if (sequenceMatch) {
        sequences.push(
...
export async function createBackup(format) {
  ...
  try {
    ...
    const verboseObjects = parsePgVerboseObjects(`${stdout}\n${stderr}`);
    const fileObjects =
      backupFormat === "dump"
        ? await parseDumpFileObjects(filepath, env)
        : await parseSqlFileObjects(filepath);

    const tables = uniqueItems([...verboseObjects.tables, ...fileObjects.tables]);
    const sequences = uniqueItems([
      ...verboseObjects.sequences,
      ...fileObjects.sequences,
    ]);
...
export async function restoreBackup(filename, customPath = null) {
  ...
  if (isDumpFormat) {
    ...
    const restoreCmd = `psql --no-password -h ${dbConfig.host} -p ${dbConfig.port} -U ${dbConfig.user} -d ${dbConfig.database} -v ON_ERROR_STOP=1 -f "${filepath}"`;

    command = `${dropSchemaCmd} && ${restoreCmd}`;
  }

  try {
    const plannedObjects = isDumpFormat
      ? await parseDumpFileObjects(filepath, env)
      : await parseSqlFileObjects(filepath);
    const { stdout, stderr } = await execAsync(command, {
      env,
    ...
    const actualObjects = parsePgVerboseObjects(`${stdout}\n${stderr}`);

    return {
      success: true,
      output: stdout || stderr,
      details: {
        tables: uniqueItems([...plannedObjects.tables, ...actualObjects.tables]),
        sequences: uniqueItems([
          ...plannedObjects.sequences,
          ...actualObjects.sequences,
        ]),
      },
    };
  } catch (error) {
